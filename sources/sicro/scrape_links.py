...
def save_links(database: SicroTrackingDatabase, source_url: str, links: list[str]) -> None:
    for link in links:
        tokens = link.split("/")
        region = tokens[-5]
        state_code = tokens[-4]
...
def scrape_links(url: str, database: SicroTrackingDatabase, visited: set[str] | None = None, depth: int = 0, max_depth: int = 5) -> list[str]:
    if visited is None:
        visited = set()
    ...
    try:
        ...
        logger.error(f"Falha ao acessar {url}: {exc}")
        return []
...
def scrape_all_regions(
    database: SicroTrackingDatabase,
    regions: list[str] | None = None,
    max_depth: int = 5,
) -> None:
    for region in (regions or sicro_regions):
        base_url = f"{sicro_base_url}/{region}"
        links = scrape_links(base_url, database, max_depth=max_depth)

        logger.info(f"Links encontrados em {base_url}:")
        for link in links:
            logger.info(link)
...
def main() -> None:
    ...
    database = SicroTrackingDatabase(db_path=str(local_db_path))

    try:
        regions = [args.region] if args.region else None
        scrape_all_regions(database, regions=regions, max_depth=args.max_depth)
        logger.info(f"URLs salvas em {local_db_path}")
    finally:
